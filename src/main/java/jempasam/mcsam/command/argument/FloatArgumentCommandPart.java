...
public class FloatArgumentCommandPart extends DecoratedCommandPart{
	
	
	
	public FloatArgumentCommandPart(String name) {
		super(name);
	}
	...
	public List<String> complete(CommandContext context) {
		...
		else return Lists.newArrayList(context.arg(0)+"0",context.arg(0)+"1",context.arg(0)+"2",context.arg(0)+"3",context.arg(0)+"4",context.arg(0)+"5",context.arg(0)+"6","7",context.arg(0)+"8",context.arg(0)+"9",context.arg(0)+".");
	}
	...
	public void execute(CommandContext context) throws CommandPartException {
		try {
			if(context.hasRemaining(1)) {
				context.addParameter(name, Float.parseFloat(context.arg(0)));
				then.execute(context.forward(1));
			}
			else ICommandPart.error("Miss float argument", context);
